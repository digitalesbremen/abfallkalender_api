...
# ------------------------------------------------------------
# GitHub Actions role for OpenTofu apply (deploy infrastructure)
# ------------------------------------------------------------

# IAM role that GitHub Actions (tag refs in this repo) may assume to run tofu plan/apply
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            # Limit to this repository and tag refs only
            "token.actions.githubusercontent.com:sub" : "repo:digitalesbremen/abfallkalender_api:ref:refs/tags/*"
          }
        }
      }
...
# Least‑privilege policy for applying this stack
# Allows managing: Lambda function and Function URL; CloudWatch Logs group; EventBridge rule/target;
# read‑only ECR for convenience; and PassRole for the Lambda execution role.
resource "aws_iam_policy" "terraform_apply" {
  name        = "GitHubActionsTofuApplyAbfallkalender"
  description = "Permissions for GitHub Actions to run tofu plan/apply for abfallkalender_api stack"
  policy      = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
...
resource "aws_iam_role_policy_attachment" "attach_terraform_apply" {
